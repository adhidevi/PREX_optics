...
//	TGaxis::SetMaxDigits(3);
	...
	int nrun = Run.size();
	int* myRun = Run.data();	
	double qedge, yhicut, theta, asym, sasym, qsq, sens;
	vector<double>Qedge, Yhicut, Theta, Asym, Sasym, Qsq, Sens;
	...
	for(int irun=0;irun<nrun;irun++){
	   filename = Form("./TextFiles/output_yhiCut_run%d.csv",myRun[irun]);
	   ...
	   while(infile>>qedge>>yhicut>>theta>>asym>>sasym>>qsq>>sens){
		Qedge.push_back(qedge);
		Yhicut.push_back(yhicut);
	...
	int sze = Qedge.size();
	for(int j=0;j<sze;j++)
	cout<<Yhicut[j]<<"\t"<<Theta[j]<<"\t"<<Asym[j]<<"\t"<<Qsq[j]<<endl;

	gr1[irun] = new TGraphErrors(sze,&Yhicut[0],&Theta[0]);
	gr2[irun] = new TGraphErrors(sze,&Yhicut[0],&Asym[0]);
	gr3[irun] = new TGraphErrors(sze,&Yhicut[0],&Qsq[0]);
	gr4[irun] = new TGraphErrors(sze,&Yhicut[0],&Sasym[0]);
	gr5[irun] = new TGraphErrors(sze,&Yhicut[0],&Sens[0]);
	...
	mg1->SetTitle("Variation of scattering angle (#theta) with yhiCut;yhiCut (m);Theta (degrees)");
	mg2->SetTitle("Variation of Asymmetry with yhiCut;yhiCut (m);Asymmetry (ppm)");
	mg3->SetTitle("Variation of Q-square with yhiCut;yhiCut (m);Qsq ((GeV/c)^{2})");
	mg4->SetTitle("Variation of Stretched Asymmetry with yhiCut;yhiCut (m);Stretched Asymmetry (ppm)");
	mg5->SetTitle("Variation of Sensitivity with yhiCut;yhiCut (m);Sensitivity");
	for(int irun=0;irun<nrun;irun++){
	   mg1->Add(gr1[irun],"lp");
	   mg2->Add(gr2[irun],"lp");
	   mg3->Add(gr3[irun],"lp");
	...
	mg1->Draw("APL");
        gPad->Update();
        TLine* q_edge = new TLine(0.022,-10000,0.022,10000);
        ...
        latex.DrawLatex(0.40,0.30,"q_edge");
	c1->cd(2);
	...
	gSystem->Exec(Form("pdfunite ./temp/gr*.pdf ./plots/yhiCut_allRunsR.pdf"));
