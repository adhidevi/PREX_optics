...
void AsymForAccYcutHi(int run, double yhiCut){
  ...
  //For the interpolation
   ...
   //plot procected x, y, and adc
      ...
      latex.DrawLatex(0.20,0.70,Form("yhiCut = %1.3f",yhiCut));     
      TLine* yhiCut_line = new TLine(yhiCut,0.0,yhiCut,hy1->GetMaximum()/3.0*2.0);
      yhiCut_line->SetLineColor(49);
      yhiCut_line->SetLineWidth(2);
      yhiCut_line->Draw();
  ...
  for(long i = 0; i < n; i++){ 
    ...
    //VDC cut and x cut
     if( thisu1==1 && thisv1==1 && thisu2==1 && thisv2==1 && xq>-0.0715 && yq<yhiCut){
 ...
 Q2->SetTitle(Form("Q^{2} (GeV/c)^{2}, yhiCut = %1.3f m",yhiCut));
 Theta->SetTitle(Form("#theta_{lab} (deg), yhiCut = %1.3f m",yhiCut));
 Asym->SetTitle(Form("Asymmetry (ppm), yhiCut = %1.3f m",yhiCut));
 SAsym->SetTitle(Form("Stretched Asym. (ppm),  yhiCut = %1.3f m",yhiCut));
 sens->SetTitle(Form("Sensitivity, yhiCut = %1.3f m",yhiCut));
 ...
 gSystem->Exec(Form("pdfunite ./temp/*.pdf ./plots/all_yhiCut_%1.3f_run%d.pdf",yhiCut,run));
 gSystem->Exec(Form("rm -rf ./temp/*.pdf"));

 ofstream outfile(Form("./TextFiles/output_yhiCut_run%d.csv",run),ios_base::app);
 outfile<<x_edge<<"\t"<<yhiCut<<"\t"<<Theta->GetMean()<<"\t"<<Asym->GetMean()<<"\t"<<SAsym->GetMean()<<"\t"<<Q2->GetMean()<<"\t"<<sens->GetMean()<<endl;
 outfile.close();
 ...
 std::cout << "Theta " << Theta->GetMean() << "  " << "<A> " << Asym->GetMean() << " " << "Stretched <A>" << SAsym->GetMean() << "   " << "Q2 " << Q2->GetMean() << "  " << sens->GetMean() << std::endl;
  ...
  for(long i = 0; i < n; i++){ 
    ...
    //VDC cut and x cut
     if( thisu1==1 && thisv1==1 && thisu2==1 && thisv2==1 && xq>-0.076 && yq<yhiCut){
