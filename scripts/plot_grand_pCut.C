void plot_grand_pCut(TString arm){
	TGaxis::SetMaxDigits(3);
	gStyle->SetTitleYOffset(1.3);
	TString runlist;
	if(arm=="RHRS")
	runlist = "./TextFiles/good_runR1.list";
	if(arm=="LHRS")
	runlist = "./TextFiles/good_runL1.list";
	...
	int nrun = Run.size();
	int* myRun = Run.data();	
	double qedge, pcut, theta, asym, sasym, qsq, sens;
	vector<double>Qedge, Pcut, Theta, Asym, Sasym, Qsq, Sens;
	...
	for(int irun=0;irun<nrun;irun++){
	   filename = Form("./TextFiles/output_pCut_run%d.csv",myRun[irun]);
	   ...
	   while(infile>>qedge>>pcut>>theta>>asym>>sasym>>qsq>>sens){
	...
	int sze = Qedge.size();
	for(int j=0;j<sze;j++)
	cout<<Pcut[j]<<"\t"<<Theta[j]<<"\t"<<Asym[j]<<"\t"<<Qsq[j]<<endl;

	gr1[irun] = new TGraphErrors(sze,&Pcut[0],&Theta[0]);
	gr2[irun] = new TGraphErrors(sze,&Pcut[0],&Asym[0]);
	gr3[irun] = new TGraphErrors(sze,&Pcut[0],&Qsq[0]);
	gr4[irun] = new TGraphErrors(sze,&Pcut[0],&Sasym[0]);
	gr5[irun] = new TGraphErrors(sze,&Pcut[0],&Sens[0]);
	...
	mg1->SetTitle(Form("Variation of scattering angle (#theta) with pCut (%s);pCut (GeV);Theta (degrees)",arm.Data()));
	mg2->SetTitle(Form("Variation of Asymmetry with pCut (%s);pCut (GeV);Asymmetry (ppm)",arm.Data()));
	mg3->SetTitle(Form("Variation of Q-square with pCut (%s);pCut (GeV);Qsq ((GeV/c)^{2})",arm.Data()));
	mg4->SetTitle(Form("Variation of Stretched Asymmetry with pCut (%s);pCut (GeV);Stretched Asymmetry (ppm)",arm.Data()));
	mg5->SetTitle(Form("Variation of Sensitivity with pCut (%s);pCut (GeV);Sensitivity",arm.Data()));
	for(int irun=0;irun<nrun;irun++){
	   mg1->Add(gr1[irun],"lp");
	   mg2->Add(gr2[irun],"lp");
	   mg3->Add(gr3[irun],"lp");
