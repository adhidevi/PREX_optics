...
void Qsq_only_pCut_weighted(int run, Float_t E){
  ...
  fract[iCut] = hpup2Cpy->Interpolate(pCut[iCut]);
 ...
 T->Draw(Form("EK_L.Q2>>Q2_%.4f",pCut[iCut]),momCut*Form("%f",fract[iCut]),"goff");
 if(iCut==4)
 ...
 gSystem->Exec(Form("pdfunite ./temp1/plot*.pdf ./plots/Qsq_pCut_W_run%d.pdf",run));
 gSystem->Exec(Form("rm -rf ./temp1/plot*.pdf"));

 gSystem->Exec(Form("rm -rf ./TextFiles/Qsq_pCut_W_run%d.csv",run));

 ofstream outfile(Form("./TextFiles/Qsq_pCut_W_run%d.csv",run));
 ofstream edgeCut("./TextFiles/Qsq_edgeCut_W_allRunsL.csv",ios_base::app);
 ...
 Q2[iCut] = new TH1F(Form("Q2_%.4f",pCut[iCut]),Form("Q^{2} (GeV/c)^{2} with pCut (run%d);Q^{2} (GeV/c)^{2}",run),150,0,0.015);
 T->Draw(Form("EK_R.Q2>>Q2_%.4f",pCut[iCut]),momCut*Form("%f",hpup2Cpy->Interpolate(pCut[iCut])),"goff");
 if(iCut==4)
 Q2[iCut]->SetLineColor(42);
 else if(iCut==6)
 Q2[iCut]->SetLineColor(9);
 ...
 gSystem->Exec(Form("rm -rf ./TextFiles/Qsq_pCut_W_run%d.csv",run));
 ofstream outfile(Form("./TextFiles/Qsq_pCut_W_run%d.csv",run));
 ofstream edgeCut("./TextFiles/Qsq_edgeCut_W_allRunsR.csv",ios_base::app);
