void combine_csv(TString arm){
	...
	ofstream outfile(Form("./TextFiles/acc_qsq_all_%sHRS_runs.csv",arm.Data()));
	...
	outfile<<"Run = "<<myRunL[irun]<<endl;
        ...
        outfile<<"q_edge"<<"\t"<<"pCut"<<"\t"<<"Qsq"<<"\t"<<"Qsq_rms"<<"\t"<<"q_dge_rel"<<"\t"<<"Acc_fr"<<endl;
        for(int j=0;j<sze;j++){
	...
	Acc_fr.clear();
        ...
        ifstream qfile(Form("./TextFiles/Qsq_edgeCut_allRuns%s.csv",arm.Data()));
	...
	if(arm=="R")
        outfile<<"RunR"<<"\t"<<"q_edge"<<"\t"<<"Qsq"<<"\t"<<"Qsq_rms"<<"\t"<<"q_dge_rel"<<"\t"<<"Acc_fr"<<endl;
	else if(arm=="L")
        outfile<<"RunL"<<"\t"<<"q_edge"<<"\t"<<"Qsq"<<"\t"<<"Qsq_rms"<<"\t"<<"q_dge_rel"<<"\t"<<"Acc_fr"<<endl;
