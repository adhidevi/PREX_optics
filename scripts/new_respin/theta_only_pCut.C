...
void theta_only_pCut(int run, Float_t E){
  ...
  //Central Angle
  double th0 = 4.84*TMath::Pi()/180;//convert to radian
 ...
 t[iCut] = new TH1F(Form("th_%.4f",pCut[iCut]),Form("#theta_{lab} with pCut (run%d);#theta_{lab} (deg);",run),150,2,8);
 T->Draw(Form("%f*TMath::ACos((%f-L.gold.ph*%f)/TMath::Sqrt(1+L.gold.th*L.gold.th+L.gold.ph*L.gold.ph))>>th_%.4f",radtodeg,cth0,sth0,pCut[iCut]),momCut,"goff");
 if(iCut==4)
 t[iCut]->SetLineColor(42);
 else if(iCut==6)
 t[iCut]->SetLineColor(9);
 else
 t[iCut]->SetLineColor(iCut+1);
 }
 TCanvas *q = new TCanvas("q","q",600,500);
 for(int iCut=0;iCut<nCut;iCut++){
 t[iCut]->Draw("sames hist");
 t[iCut]->Scale(1.0/t[iCut]->GetMaximum());
 }
 gPad->Update();
 TPaveStats* statsT[nCut];
 for(int iCut=0;iCut<nCut;iCut++){
 statsT[iCut] = (TPaveStats*)t[iCut]->FindObject("stats");
 }
 statsT[0]->SetY2NDC(0.90);
 statsT[0]->SetY1NDC(0.75);
 statsT[1]->SetY2NDC(0.75);
 statsT[1]->SetY1NDC(0.60);
 statsT[2]->SetY2NDC(0.60);
 statsT[2]->SetY1NDC(0.45);
 statsT[3]->SetY2NDC(0.45);
 statsT[3]->SetY1NDC(0.30);

 statsT[0]->SetX2NDC(0.90);
 statsT[0]->SetX1NDC(0.70);
 statsT[1]->SetX2NDC(0.90);
 statsT[1]->SetX1NDC(0.70);
 statsT[2]->SetX2NDC(0.90);
 statsT[2]->SetX1NDC(0.70);
 statsT[3]->SetX2NDC(0.90);
 statsT[3]->SetX1NDC(0.70);

 statsT[4]->SetY2NDC(0.90);
 statsT[4]->SetY1NDC(0.75);
 statsT[5]->SetY2NDC(0.75);
 statsT[5]->SetY1NDC(0.60);
 statsT[6]->SetY2NDC(0.60);
 statsT[6]->SetY1NDC(0.45);

 statsT[4]->SetX2NDC(0.33);
 statsT[4]->SetX1NDC(0.13);
 statsT[5]->SetX2NDC(0.33);
 statsT[5]->SetX1NDC(0.13);
 statsT[6]->SetX2NDC(0.33);
 statsT[6]->SetX1NDC(0.13);
 for(int iCut=0;iCut<nCut;iCut++){
 if(iCut==4)
 statsT[iCut]->SetTextColor(42);
 else if(iCut==6)
 statsT[iCut]->SetTextColor(9);
 else
 statsT[iCut]->SetTextColor(iCut+1);
 }
 gPad->Modified();
 q->SaveAs("./temp1/plot2.pdf");

 gSystem->Exec(Form("pdfunite ./temp1/plot*.pdf ./plots/theta_pCut_run%d.pdf",run));
 gSystem->Exec(Form("rm -rf ./temp1/plot*.pdf"));

 gSystem->Exec(Form("rm -rf ./TextFiles/theta_pCut_run%d.csv",run));
 ofstream outfile(Form("./TextFiles/theta_pCut_run%d.csv",run));
 ofstream edgeCut("./TextFiles/theta_edgeCut_allRunsL.csv",ios_base::app);

 for(int iCut=0;iCut<nCut;iCut++){
 if(EdgeP==pCut[iCut])
 edgeCut<<EdgeP<<"\t"<<pCut[iCut]<<"\t"<<t[iCut]->GetMean()<<"\t"<<t[iCut]->GetRMS()<<"\t"<<epfit->GetParameter(1)-pCut[iCut]<<"\t"<<hpup2Cpy->Interpolate(pCut[iCut])<<endl;
 else
 outfile<<EdgeP<<"\t"<<pCut[iCut]<<"\t"<<t[iCut]->GetMean()<<"\t"<<t[iCut]->GetRMS()<<"\t"<<epfit->GetParameter(1)-pCut[iCut]<<"\t"<<hpup2Cpy->Interpolate(pCut[iCut])<<endl;
}
 outfile.close();
 edgeCut.close();
}else{
 ...
 t[iCut] = new TH1F(Form("th_%.4f",pCut[iCut]),Form("#theta_{lab} with pCut (run%d);#theta_{lab} (GeV/c)^{2}",run),150,2,8);
 T->Draw(Form("%f*TMath::ACos((%f+R.gold.ph*%f)/TMath::Sqrt(1+R.gold.th*R.gold.th+R.gold.ph*R.gold.ph))>>th_%.4f",radtodeg,cth0,sth0,pCut[iCut]),momCut,"goff");
 if(iCut==4)
 t[iCut]->SetLineColor(42);
 else if(iCut==6)
 t[iCut]->SetLineColor(9);
 ...
 statsT[4]->SetX2NDC(0.35);
 statsT[4]->SetX1NDC(0.15);
 statsT[5]->SetX2NDC(0.35);
 statsT[5]->SetX1NDC(0.15);
 statsT[6]->SetX2NDC(0.35);
 statsT[6]->SetX1NDC(0.15);
 for(int iCut=0;iCut<nCut;iCut++){
 if(iCut==4)
 statsT[iCut]->SetTextColor(42);
 else if(iCut==6)
 ...
 gSystem->Exec(Form("rm -rf ./TextFiles/theta_pCut_run%d.csv",run));
 ofstream outfile(Form("./TextFiles/theta_pCut_run%d.csv",run));
 ofstream edgeCut("./TextFiles/theta_edgeCut_allRunsR.csv",ios_base::app);
