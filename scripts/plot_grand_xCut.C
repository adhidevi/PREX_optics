void plot_grand_xCut(){
	...
	int nrun = Run.size();
	int* myRun = Run.data();	
	double qedge, xcut, theta, asym, sasym, qsq, sens;
	vector<double>Qedge, Xcut, Theta, Asym, Sasym, Qsq, Sens;
	...
	for(int irun=0;irun<nrun;irun++){
	   filename = Form("./TextFiles/output_xCut_run%d.csv",myRun[irun]);
	   ...
	   while(infile>>qedge>>xcut>>theta>>asym>>sasym>>qsq>>sens){
	...
	int sze = Qedge.size();
	for(int j=0;j<sze;j++)
	cout<<Xcut[j]<<"\t"<<Theta[j]<<"\t"<<Asym[j]<<"\t"<<Qsq[j]<<endl;

	gr1[irun] = new TGraphErrors(sze,&Xcut[0],&Theta[0]);
	gr2[irun] = new TGraphErrors(sze,&Xcut[0],&Asym[0]);
	gr3[irun] = new TGraphErrors(sze,&Xcut[0],&Qsq[0]);
	gr4[irun] = new TGraphErrors(sze,&Xcut[0],&Sasym[0]);
	gr5[irun] = new TGraphErrors(sze,&Xcut[0],&Sens[0]);
	...
	mg1->SetTitle("Variation of scattering angle (#theta) with xCut;xCut (m);Theta (degrees)");
	mg2->SetTitle("Variation of Asymmetry with xCut;xCut (m);Asymmetry (ppm)");
	mg3->SetTitle("Variation of Q-square with xCut;xCut (m);Qsq ((GeV/c)^{2})");
	mg4->SetTitle("Variation of Stretched Asymmetry with xCut;xCut (m);Stretched Asymmetry (ppm)");
	mg5->SetTitle("Variation of Sensitivity with xCut;xCut (m);Sensitivity");
	for(int irun=0;irun<nrun;irun++){
	   mg1->Add(gr1[irun],"lp");
	   mg2->Add(gr2[irun],"lp");
	   mg3->Add(gr3[irun],"lp");
	...
	mg1->Draw("APL");
	gPad->Update();
	TLine* q_edge = new TLine(-0.076,-10000,-0.076,10000);
	...
	latex.DrawLatex(0.70,0.85,"q_edge");
	c1->cd(2);
	...
	gSystem->Exec(Form("pdfunite ./temp/gr*.pdf ./plots/xCut_allRunsR.pdf"));
