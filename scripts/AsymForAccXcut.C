...
void AsymForAccXcut(int run, double xCut){
  ...
  for(long i = 0; i < n; i++){ 
    ...
    //VDC cut and x cut
     if( thisu1==1 && thisv1==1 && thisu2==1 && thisv2==1 && xq > xCut){
 ...
 gSystem->Exec(Form("pdfunite ./temp/*.pdf ./plots/all_xCut_%1.3f_run%d.pdf",xCut,run));
 gSystem->Exec(Form("rm -rf ./temp/*.pdf"));

 ofstream outfile(Form("./TextFiles/output_xCut_run%d.csv",run),ios_base::app);
 outfile<<x_edge<<"\t"<<xCut<<"\t"<<Theta->GetMean()<<"\t"<<Asym->GetMean()<<"\t"<<SAsym->GetMean()<<"\t"<<Q2->GetMean()<<"\t"<<sens->GetMean()<<endl;
 outfile.close();
