...
void CheckVertexAsym(){
     ...
     TString acc_shape = "smear";  // option: original, smear, box, gaus
	...
	accp_angle_p[ii] = mean0+(accp_angle[ii]-mean0)*(1.+rms_percent); // +rms_percent
     ...
     if(check_ACC){
	  ...
	  if(acc_shape == "smear" ) thisACC = FindACC(thisAngle,accp_angle_p,accp,accp_err,100);  // original acceptance function
	...
	hq2_e_acc->Fill(thisQ2,thisXS*omega*thisACC);   // elemetal Q2 with the acceptance function applied
     ...
     c2->SaveAs("./temp/th_q2.pdf");
     if(check_ACC){
       TCanvas *c3 = new TCanvas("c3","c3",1500,1500);
       hACC_check->Draw("hist");
       c3->SaveAs("./temp/acceptanceFn.pdf");
     }
