...
void CheckVertexAsym(){
     ...
     T->Add("/lustre/expphy/volatile/halla/parity/ryanrich/Ebeam953/TargNominal/Zero1_SandwichLHRS_PREX_0.0_1.root");
     T->Add("/lustre/expphy/volatile/halla/parity/ryanrich/Ebeam953/TargNominal/Zero1_SandwichLHRS_PREX_0.0_2.root");
     T->Add("/lustre/expphy/volatile/halla/parity/ryanrich/Ebeam953/TargNominal/Zero1_SandwichLHRS_PREX_0.0_3.root");
     T->Add("/lustre/expphy/volatile/halla/parity/ryanrich/Ebeam953/TargNominal/Zero1_SandwichLHRS_PREX_0.0_4.root");
     T->Add("/lustre/expphy/volatile/halla/parity/ryanrich/Ebeam953/TargNominal/Zero1_SandwichLHRS_PREX_0.0_5.root");
     ...
     double momcut = 2.2;//MeV from el_peak
     TString dpcut = Form("(%f-p_ztarg)<%f",p_peak,momcut);
     TCut DP = Form("%s",dpcut.Data());

     // dp cut for the incident events
     double delta_p_percent = 3./100.;
	...
	hq2_e_acc->Fill(thisQ2,thisXS*omega*thisACC);   // elemetal Q2 with the acceptance function applied
     ...
     gPad->Modified();
     c1->SaveAs("./temp/asym_with_acc.pdf");
     ...
     TLegend *leg1 = new TLegend(0.55,0.70,0.70,0.8);
     leg1->AddEntry(hth_origin,"origin","L");
     leg1->AddEntry(hth_e_acc,"elemental","L");
     leg1->Draw();
     gPad->Update();
     TPaveStats* statT1 = (TPaveStats*)hth_origin->FindObject("stats");
     TPaveStats* statT2 = (TPaveStats*)hth_e_acc->FindObject("stats");
     statT1->SetY1NDC(0.90);
     statT1->SetY2NDC(0.75);
     statT2->SetY1NDC(0.75);
     statT2->SetY2NDC(0.60);
     statT1->SetTextColor(1);
     statT2->SetTextColor(2);
     gPad->Modified();
     ...
     TLegend *leg2 = new TLegend(0.60,0.70,0.75,0.8);
     leg2->AddEntry(hq2_origin,"origin","L");
     leg2->AddEntry(hq2_e_acc,"elemental","L");
     leg2->Draw();
     gPad->Update();
     TPaveStats* statQ1 = (TPaveStats*)hq2_origin->FindObject("stats");
     TPaveStats* statQ2 = (TPaveStats*)hq2_e_acc->FindObject("stats");
     statQ1->SetY1NDC(0.90);
     statQ1->SetY2NDC(0.75);
     statQ2->SetY1NDC(0.75);
     statQ2->SetY2NDC(0.60);
     statQ1->SetTextColor(1);
     statQ2->SetTextColor(2);
     gPad->Modified();
     c2->SaveAs("./temp/th_qsq_with_acc.pdf");
