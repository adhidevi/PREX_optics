...
void AsymL_innerHole_test(int run,double sept,double ADC,double th0, double dp, double off){
...
// T1->SetBranchAddress("x_zsieve_tr",&); T1->SetBranchAddress("y_zsieve_tr",&
 ...
 thisMom = ((thismom-951)/951 - 0.05*thisTh);
  ...
  if( thisXfp!=-333.&&!(thisTh>-0.0038&&thisTh<0.0032&&thisPh>-0.0175&&thisPh<-0.0095) && CollimatorL(thisXcol,thisYcol) && UpPlane(thisxu1,thisyu1,thisxu2,thisyu2,1) &&
 ...
 ofstream outfile("./TextFiles/inner_hole_cut.csv");
 ...
 outfile.close();
 gSystem->Exec(Form("pdfunite ./temp/*.pdf ./plots/inner_hole_cut.pdf"));
