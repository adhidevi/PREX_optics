...
void AsymL(int run,double sept,double ADC,double th0, double dp, double off, double Ebeam, double YColl_offset){
...
double thisEvt;
 ...
 ofstream outfile(Form("./TextFiles/septum_scan_Ycoll_scan_newReplay.csv"),ios_base::app);

// outfile<<Form("septum_%.1f_Ycoll_%.3f",sept,YColl_offset)<<endl;
// outfile<<Form("septum_%.1f_Ycoll_%.3f",sept,YColl_offset)<<"\t"<<lab[0]->GetMean()<<"\t"<<lab[0]->GetRMS()<<"\t"<<lab[1]->GetMean()<<"\t"<<lab[1]->GetRMS()<<"\t"<<qsq[0]->GetMean()<<"\t"<<qsq[0]->GetRMS()<<"\t"<<qsq[1]->GetMean()<<"\t"<<qsq[1]->GetRMS()<<"\t"<<asym[0]->GetMean()<<"\t"<<asym[0]->GetRMS()<<"\t"<<asym[1]->GetMean()<<"\t"<<asym[1]->GetRMS()<<endl;
// outfile<<"data/sim"<<"\t"<<"-"<<"\t"<<"-"<<"\t"<<lab[0]->GetMean()/lab[1]->GetMean()*100<<"\t"<<lab[0]->GetRMS()/lab[1]->GetRMS()*100<<"\t"<<"-"<<"\t"<<"-"<<"\t"<<qsq[0]->GetMean()/qsq[1]->GetMean()*100<<"\t"<<qsq[0]->GetRMS()/qsq[1]->GetRMS()*100<<"\t"<<"-"<<"\t"<<"-"<<"\t"<<asym[0]->GetMean()/asym[1]->GetMean()*100<<"\t"<<asym[0]->GetRMS()/asym[1]->GetRMS()*100<<endl;
