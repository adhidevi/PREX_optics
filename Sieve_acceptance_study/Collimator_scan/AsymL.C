...
void AsymL(int run,double sept,double ADC,double th0, double peakpos, double simMomCut, double Ebeam, double YColl_offset){
...
double thisEvt;
  ...
  T->Add(Form("/chafs1/work1/prex_counting/Qsq_Oct19/prexLHRS_%i_-1*.root",run));

  TChain *T1 = new TChain("T");
  for(int i = 1; i < 6; i++){ T1->Add(Form("/w/halla-scifs17exp/parity/disk1/ryanrich/Ebeam953/TargNominal/Zero1_SandwichLHRS_PREX_%.1f_%i.root",sept,i)); }
 ...
 T1->GetEntry(j);
  ...
  if( thisXvdc!=-333. && CollimatorL(thisXcol,thisYcol+YColl_offset) && UpPlane(thisxu1,thisyu1,thisxu2,thisyu2,1) &&
  DownPlane(thisxd1,thisyd1,thisxd2,thisyd2,thisxd3,thisyd3,thisxd4,thisyd4,thisxd5,thisyd5,thisxd6,thisyd6,thisxd7,thisyd7,thisxd8,thisyd8,thisxd9,thisyd9,1)&& ((thismom -50*thisTh) > ((Ebeam - 0.001 - simMomCut)*1000))){
 ...
 qsq_v->Fill(Qsq_v[vi],Rate[vi]); asym_v->Fill(Asym_v[vi],Rate[vi]); lab_v->Fill(Lab_v[vi],Rate[vi]);
  }
 ...
 ofstream outfile(Form("./TextFiles/septum_scan_Ycoll_scan_noefactsim_run%d.csv",run),ios_base::app);
// ofstream outfile(Form("./TextFiles/scattering_angle_scan_run%d.csv",run),ios_base::app);
 ...
 ofstream outfile_v(Form("./TextFiles/vertex_septum_scan_Ycoll_scan_noefactsim_4806deg_run%d.csv",run),ios_base::app);
// ofstream outfile_v(Form("./TextFiles/vertex_scattering_angle_scan_run%d.csv",run),ios_base::app);

 outfile_v<<sept<<"\t"<<YColl_offset<<"\t"<<lab_v->GetMean()<<"\t"<<lab[1]->GetMean()<<"\t"<<lab_v->GetMean()/lab[1]->GetMean()*100.<<"\t"<<qsq_v->GetMean()<<"\t"<<qsq[1]->GetMean()<<"\t"<<qsq_v->GetMean()/qsq[1]->GetMean()*100.<<"\t"<<asym_v->GetMean()<<"\t"<<asym[1]->GetMean()<<"\t"<<asym_v->GetMean()/asym[1]->GetMean()*100.<<"\t"<<lab_v->GetRMS()<<"\t"<<lab[1]->GetRMS()<<"\t"<<lab_v->GetRMS()/lab[1]->GetRMS()<<"\t"<<qsq_v->GetRMS()<<"\t"<<qsq[1]->GetRMS()<<"\t"<<qsq_v->GetRMS()/qsq[1]->GetRMS()<<"\t"<<asym_v->GetRMS()<<"\t"<<asym[1]->GetRMS()<<"\t"<<asym_v->GetRMS()/asym[1]->GetRMS()<<endl;

 outfile_v.close();

 gSystem->Exec(Form("pdfunite ./temp/*.pdf ./plots/vertex_septum_noefactsim_%.1f_Ycoll_%.3f_run%d.pdf",sept,YColl_offset,run));
