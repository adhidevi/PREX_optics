...
void CompareRHRS(int run, double sept, double ADC, double th0){ 
  ...
  for(int i = 0; i < 2; i++){ 
    thtg[i] = new TH1F(Form("thtg[%i]",i),"RHRS Apparent #theta_{tg}",150,-0.06,0.06); 
    phtg[i] = new TH1F(Form("phtg[%i]",i),"RHRS Apparent #phi_{tg}",150,-0.03,0.03);
  ...
  for(int i = 1; i < 6; i++) { T1->Add(Form("../septmistune/Rootfiles/SandwichRHRS_PREX_%.1f_%i.root",sept,i)); }


  //Data cuts
  TCut vdc_cut = "R.vdc.u1.nclust==1&&R.vdc.v1.nclust==1&&R.vdc.u2.nclust==1&&R.vdc.v2.nclust==1";
  TCut adc_cut = Form("P.upQadcR > %f",ADC);
  ...
  TCut col_cut = "CollimatorR(x_col_tr,y_col_tr)";
  ...
  //For RHRS -- change sign
  //Should this have an offset applied too since I compare to simulation? I won't include it for now
  T->SetAlias("th",Form("%f*TMath::ACos((%f+%f*R.gold.ph)/TMath::Sqrt(1+R.gold.th[0]*R.gold.th+R.gold.ph*R.gold.ph))",r2d,cth0,sth0));
  T->SetAlias("Qsq",Form("2*0.951*R.gold.p*(1 -((%f+%f*R.gold.ph)/TMath::Sqrt(1+R.gold.th[0]*R.gold.th+R.gold.ph[0]*R.gold.ph[0])))",cth0,sth0));
  ...
  T->Project(thtg[0]->GetName(),"R.gold.th[0]",cut_dat);
  T->Project(phtg[0]->GetName(),Form("R.gold.ph[0]+%f",Off),cut_dat);//for the data comparison
