...
double thisdPdat, thisEvt;
  ...
  T->Add(Form("/w/halla-scifs17exp/parity/disk1/ryanrich/prex2Rootfiles/prexRHRS_%i_-1.root",run));


  TChain *T1 = new TChain("T");
  for(int i = 1; i < 6; i++){ T1->Add(Form("/w/halla-scifs17exp/parity/disk1/ryanrich/sandwich/SandwichRHRS_PREX_%.1f_%i.root",sept,i)); }
 ...
 T1->GetEntry(j);
  ...
  if( thisXfp!=-333. && CollimatorR(thisXcol,thisYcol) && UpPlane(thisxu1,thisyu1,thisxu2,thisyu2,0) &&
  ...
  Asym_v.push_back(thisAsym_v/1000.);
 ...
 for(int k = 0; k < Qsq.size(); k++){ 
	qsq[1]->Fill(Qsq[k],Rate[k]); lab[1]->Fill(Lab[k],Rate[k]); asym[1]->Fill(Asym[k],Rate[k]); 
        qsq_v->Fill(Qsq_v[k],Rate[k]); lab_v->Fill(Lab_v[k],Rate[k]); asym_v->Fill(Asym_v[k],Rate[k]); }
