...
void kAsymL(int run=2294,double sept=0.0,double ADC=480,double th0=4.806, double peakpos = 0.9495, double simMomCut = 0.0022,double pinch=0.0, int iprint = 0){

  double E0 = 0.9534;
  // expected eloss in lead target about 1 MeV
  double efact = (E0-0.001)/peakpos;
  // Sim E offset  // 953.4 - 1
  double efactsim = 953.4/951.0;
  ...
  for(int i = 0; i < 2; i++){ 
    ...
    if (i == 0) {
      ...
      //      hmom[i] = new TH1F(Form("hmom[%i]",i),"LHRS Apparent Momentum Data",200,0.900,0.960);
      ...
      //hmom[i] = new TH1F(Form("hmom[%i]",i),Form("G4HRS-L Apparent Momentum Septum %.1f",sept),200,0.900,0.960);
  ...
  //T1->Add(Form("./simRootfiles/SandwichLHRS_PREX_%.1f_%i.root",sept,1));
  ...
  for(int j = 0; j < m; j++){ 
    ...
    scfp = thisXfp!=-333.;
    sccol = CollimatorL(thisXcol,thisYcol);
  ...
  lab[0]->Draw("HIST same");
  leg->Draw();
  if(iprint) {
    if (pinch!= 0.0) {
      c1->Print(Form("./fig/Run%4i_sept%.1f_Theta_pinch%.1f.pdf",run,sept,pinch));
    }else{
      c1->Print(Form("./fig/Run%4i_sept%.1f_Theta.pdf",run,sept));
    }
  }	    
  ...
  qsq[0]->Draw("HIST same");
  leg->Draw();
  if(iprint) {
    if (pinch!= 0.0) {
      c2->Print(Form("./fig/Run%4i_sept%.1f_Q2_pinch%.1f.pdf",run,sept,pinch));
    }else{
      c2->Print(Form("./fig/Run%4i_sept%.1f_Q2.pdf",run,sept));
    }
  }	    
  ...
  if(iprint) {
    if (pinch!= 0.0) {
      c3->Print(Form("./fig/Run%4i_sept%.1f_Asym_pinch%.1f.pdf",run,sept,pinch));
    }else{
      c3->Print(Form("./fig/Run%4i_sept%.1f_Asym.pdf",run,sept));
    }
  }	    
  ...
  hasym_v->Draw("HIST same");
  leg2->Draw();
  if(iprint) {
    if (pinch!= 0.0) {
      c5->Print(Form("./fig/Run%4i_sept%.1f_vertex_pinch%.1f.pdf",run,sept,pinch));
    }else{
      c5->Print(Form("./fig/Run%4i_sept%.1f_vertex.pdf",run,sept));
    }
  }	    
  ...
  hmom[0]->Draw("HIST same");
  leg->Draw();
  
  c4->cd(3);
  hphtg[1]->Draw("hist");
  hphtg[0]->Scale(hphtg[1]->Integral()/hphtg[0]->Integral());
  hphtg[0]->Draw("HIST same");
  leg->Draw();
  
  c4->cd(4);
  hthtg[1]->Draw("hist");
  hthtg[0]->Scale(hthtg[1]->Integral()/hthtg[0]->Integral());
  hthtg[0]->Draw("HIST same");
  leg->Draw();
  if(iprint) {
    if (pinch!= 0.0) {
      c4->Print(Form("./fig/Run%4i_sept%.1f_kin_pinch%.1f.pdf",run,sept,pinch));
    }else{
      c4->Print(Form("./fig/Run%4i_sept%.1f_kin.pdf",run,sept));
    }
  }	    
  ...
  if(iprint) {
    if (pinch!= 0.0) {
      c6->Print(Form("./fig/Run%4i_sept%.1f_mom_pinch%.1f.pdf",run,sept,pinch));
    }else{
      c6->Print(Form("./fig/Run%4i_sept%.1f_mom.pdf",run,sept));
    }
  }	    
