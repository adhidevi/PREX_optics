...
void SieveSpecR(int n, TString pl, double sept){

gStyle->SetOptStat(0);


TChain *T = new TChain("T");
for(int i = 1; i < 6; i++){ T->Add(Form("../septmistune/Rootfiles/rhrsPREX/septscaleRHRS_PREX_%.1f_%i.root",i)); }
//T->Add(Form("../septmistune/Rootfiles/rhrsPREX/septscaleRHRS_PREX_%.1f_1.root",sept));
//T->Add(Form("../septmistune/Rootfiles/rhrsPREX/septscaleRHRS_PREX_%.1f_1_1.root",sept));


TCut totalCut;
TCut specCut = Form("x_%s_tr!=-333.",pl.Data());
TCut colCut = "CollimatorR(x_col_tr,y_col_tr)";
TCut sieveCut = "sieveR(x_zsieve_tr,y_zsieve_tr)!=1";
TCut septCut;
...
//TH2F *hplanes = new TH2F("hplanes",Form("Sieve Holes Septum Scale %.1f Percent",sept),n,-0.05,0.05,n,-0.06,0.06);
 ...
 hplanes->GetYaxis()->SetTitle("theta");
  ...
  c->SaveAs(Form("SieveHolesPassCollR_%s.png",pl.Data()));
//  c->SaveAs(Form("sieveHoles_Scan%.1fper.png",sept));  
