...
//  char lcut[400], rcut[400];
  ...
  T->Add(Form("/chafs1/work1/prex_counting/Qsq_Oct22/prexLHRS_%d_-1*.root",run));
     ...
     TCut lcut = Form("((P.evtypebits&2)==2&&L.vdc.u1.nclust==1&&L.vdc.v1.nclust==1&&L.vdc.u2.nclust==1&&L.vdc.v2.nclust==1&&L.gold.th>-0.08&&L.gold.th<0.08&&L.gold.ph>-0.05&&L.gold.ph<0.05&&L.gold.dp>-0.04&&L.gold.dp<0.002&&P.upQadcL>%f)",upADCcut);
 //    TCut lcut = Form("((P.evtypebits&2)==2&&L.vdc.u1.nclust==1&&L.vdc.v1.nclust==1&&L.vdc.u2.nclust==1&&L.vdc.v2.nclust==1&&P.upQadcL>%f)",upADCcut);
     ...
     T->Draw("L.gold.th:L.gold.ph>>h2",Form("P.upQadcL>%f",upADCcut),"colz");
     ofstream outfile("./TextFiles/thetaL.csv",ios_base::app);
     outfile<<hl->GetMean()<<"\t"<<hl->GetRMS()<<endl;
     outfile.close();
  ...
  T->Add(Form("/chafs1/work1/prex_counting/RootFilesQsq/prexRHRS_%d_-1*.root",run));
    ...
    // Note sign of R.gold.ph is, relatively, negative here, in comparison with L-HRS.
     ...
     TCut rcut = Form("((P.evtypebits&2)==2&&R.vdc.u1.nclust==1&&R.vdc.v1.nclust==1&&R.vdc.u2.nclust==1&&R.vdc.v2.nclust==1&&R.gold.th>-0.08&&R.gold.th<0.08&&R.gold.ph>-0.05&&R.gold.ph<0.04&&R.gold.dp>-0.04&&R.gold.dp<0.002&&P.upQadcR>%f)",upADCcut);
//     TCut rcut = Form("((P.evtypebits&2)==2&&R.vdc.u1.nclust==1&&R.vdc.v1.nclust==1&&R.vdc.u2.nclust==1&&R.vdc.v2.nclust==1&&P.upQadcR>%f)",upADCcut);
    ...
    T->Draw("R.gold.th:R.gold.ph>>h2","P.upQadcR>500","colz");
     ofstream outfile("./TextFiles/thetaR.csv",ios_base::app);
     outfile<<hr->GetMean()<<"\t"<<hr->GetRMS()<<endl;
     outfile.close();
     c1->SaveAs(Form("./temp/theta_run%d.pdf",run));
  }
}
