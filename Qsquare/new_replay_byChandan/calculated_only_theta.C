...
#include <fstream>
#include <iostream>
void calculated_only_theta(int runL, int runR, TString target, TString date, Float_t Ebeam){ 
  gROOT->Reset();
  ...
  TGaxis::SetMaxDigits(3);
      ...
      TH1F* hThL = new TH1F("hThL",Form("Theta w/ T1 trigger (run%d, %d);Theta (GeV/c)^{2}",runL,runR), 100, 2, 8);
      TH1F* hThR = new TH1F("hThR",Form("Theta w/ T1 trigger (run%d, %d);Theta (GeV/c)^{2}",runL,runR), 100, 2, 8);
      ...
      TL->Draw(Form("%f*TMath::ACos((%f-L.gold.ph*%f)/(TMath::Sqrt(1.0+L.gold.th*L.gold.th+L.gold.ph*L.gold.ph)))>>hThL",radtodeg,c0L,s0L),cut_wadcL,"hist");
      TR->Draw(Form("%f*TMath::ACos((%f+R.gold.ph*%f)/(TMath::Sqrt(1.0+R.gold.th*R.gold.th+R.gold.ph*R.gold.ph)))>>hThR",radtodeg,c0R,s0R),cut_wadcR,"sames hist");
      hThL->Scale(1./hThL->Integral());
      hThR->Scale(1./hThR->Integral());
      gPad->Update();

      TPaveStats* left = (TPaveStats*)hThL->FindObject("stats");
      TPaveStats* right = (TPaveStats*)hThR->FindObject("stats");
      left->SetY1NDC(0.90);
      left->SetY2NDC(0.75);
      right->SetY1NDC(0.75);
      right->SetY2NDC(0.60);
      ...
      ofstream outLR("./TextFiles/theta_LR.csv",ios_base::app);
      outLR<<date.Data()<<"\t"<<target.Data()<<"\t"<<runL<<"\t"<<hThL->GetMean()<<"\t"<<hThL->GetMeanError()<<"\t"<<runR<<"\t"<<hThR->GetMean()<<"\t"<<hThR->GetMeanError()<<endl;
      outLR.close();

      c1->SaveAs(Form("./temp/Theta_comp_LR_%d_run%d.pdf",runL,runR));
}
