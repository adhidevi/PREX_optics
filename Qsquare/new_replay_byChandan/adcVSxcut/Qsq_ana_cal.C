...
#include <fstream>
#include <iostream>
void Qsq_ana_cal(int run, TString target, TString date, Float_t Ebeam){ 
  gROOT->Reset();
...
//      T->Add(Form("/lustre19/expphy/volatile/halla/parity/devi/rootfiles/newDatabase/prexLHRS_%d_-1*.root",run));
      T->Add(Form("/lustre19/expphy/volatile/halla/parity/prex2-optics/prexLHRS_%d_-1*.root",run));
...
//      TCut cut_wadc = Form("((P.evtypebits&2)==2&&L.vdc.u1.nclust==1&&L.vdc.v1.nclust==1&&L.vdc.u2.nclust==1&&L.vdc.v2.nclust==1&&P.upQadcL>%f)",upADCcut);
      ...
      TH1F* hp = new TH1F("hp","hp",200,0.91,0.96);
      T->Draw("L.gold.p>>hp",cut_wadc,"goff");
      double peakpos = hp->GetBinCenter(hp->GetMaximumBin());
      ...
      T->Draw(Form("2*%f*%f*L.gold.p*(1-((%f-L.gold.ph*%f)/(TMath::Sqrt(1.0+L.gold.th*L.gold.th+L.gold.ph*L.gold.ph))))>>hQsq_cal",Ebeam,efact,c0,s0),cut_wadc,"sames");
      ...
      T->Add(Form("/lustre19/expphy/volatile/halla/parity/devi/rootfiles/newDatabase/prexRHRS_%d_-1*.root",run));
      gStyle->SetStatX(0.90);
      gStyle->SetStatY(0.90);
      gStyle->SetStatW(0.25);
      gStyle->SetStatH(0.10);
...
//      TCut cut_wadc = Form("((P.evtypebits&2)==2&&R.vdc.u1.nclust==1&&R.vdc.v1.nclust==1&&R.vdc.u2.nclust==1&&R.vdc.v2.nclust==1&&P.upQadcR>%f)",upADCcut);
      ...
      TH1F* hp = new TH1F("hp","hp",200,0.91,0.96);
      T->Draw("R.gold.p>>hp",cut_wadc,"goff");
      double peakpos = hp->GetBinCenter(hp->GetMaximumBin());
      double efact = (Ebeam-0.001)/peakpos;
      ...
      T->Draw(Form("2*%f*%f*R.gold.p*(1-((%f+R.gold.ph*%f)/(TMath::Sqrt(1.0+R.gold.th*R.gold.th+R.gold.ph*R.gold.ph))))>>hQsq_cal",Ebeam,efact,c0,s0),cut_wadc,"sames");
