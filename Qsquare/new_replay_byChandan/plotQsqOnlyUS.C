...
//  int run = (int)T->GetMaximum("fEvtHdr.fRun");
      TChain *T = new TChain("T");
  if(run < 10000){
      ofstream outfile("./TextFiles/out_fileL.txt",ios_base::app);
      //LHRS
      ...
      TCut cut_wadc = Form("((P.evtypebits&2)==2&&L.vdc.u1.nclust==1&&L.vdc.v1.nclust==1&&L.vdc.u2.nclust==1&&L.vdc.v2.nclust==1&&L.gold.th>-0.08&&L.gold.th<0.08&&L.gold.ph>-0.05&&L.gold.ph<0.04&&L.gold.dp<-0.002&&P.upQadcL>%f)",upADCcut);
//      TCut cut_wadc = Form("((P.evtypebits&2)==2&&L.vdc.u1.nclust==1&&L.vdc.v1.nclust==1&&L.vdc.u2.nclust==1&&L.vdc.v2.nclust==1&&P.upQadcL>%f)",upADCcut);
      ...
      TH1F* hQsq = new TH1F("hQsq",Form("LHRS Qsq from T-tree w/ T1 trigger (run%d,%s)",run,target.Data()), 100, 0.002, 0.013);

      T->Draw("EK_L.Q2>>hQsq", cut_wadc);
      hQsq->SetXTitle("Qsq(GeV/c)^{2}");
      hQsq->SetLineColor(kBlue);
      hQsq->Draw();
      outfile<<date.Data()<<"\t"<<target.Data()<<"\t"<<run<<"\t"<<hQsq->GetMean()<<"\t"<<hQsq->GetMeanError()<<endl;
      c1->SaveAs(Form("./temp/LHRS_Qsq_%s_run%d.pdf",target.Data(),run));
    }else{
      ofstream outfile("./TextFiles/out_fileR.txt",ios_base::app);
      // RHRS
      ...
      TCut cut_wadc = Form("((P.evtypebits&2)==2&&R.vdc.u1.nclust==1&&R.vdc.v1.nclust==1&&R.vdc.u2.nclust==1&&R.vdc.v2.nclust==1&&R.gold.th>-0.08&&R.gold.th<0.08&&R.gold.ph>-0.05&&R.gold.ph<0.04&&R.gold.dp<-0.002&&P.upQadcR>%f)",upADCcut);
//      TCut cut_wadc = Form("((P.evtypebits&2)==2&&R.vdc.u1.nclust==1&&R.vdc.v1.nclust==1&&R.vdc.u2.nclust==1&&R.vdc.v2.nclust==1&&P.upQadcR>%f)",upADCcut);
      ...
      gStyle->SetStatW(0.35);
      gStyle->SetStatH(0.08);
      TH1F* hQsq = new TH1F("hQsq", Form("RHRS Qsq from T-tree w/ T1 trigger (run%d,%s)",run,target.Data()), 100, 0.002, 0.013);
