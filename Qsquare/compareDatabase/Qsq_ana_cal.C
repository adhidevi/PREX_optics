...
#include <fstream>
#include <iostream>
void Qsq_ana_cal(int run_num, TString target){ 
  gROOT->Reset();
  ...
  if(run_num < 10000){
      ...
      TH1F* hQsq = new TH1F("hQsq",Form("LHRS Qsq w/ T1 trigger (run%d,%s);Qsq (GeV/c)^{2}",run_num,target.Data()), 100, 0.002, 0.013);
      TH1F* hQsq_cal = new TH1F("hQsq_cal",Form("LHRS Qsq w/ T1 trigger (run%d,%s);Qsq (GeV/c)^{2}",run_num,target.Data()), 100, 0.002, 0.013);
      ...
      c1->SaveAs(Form("./temp/LHRS_Qsq_comp_%s_run%d.pdf",target.Data(),run_num));
...
//      TCut cut_wadc = Form("((P.evtypebits&2)==2&&R.vdc.u1.nclust==1&&R.vdc.v1.nclust==1&&R.vdc.u2.nclust==1&&R.vdc.v2.nclust==1&&R.gold.th>-0.08&&R.gold.th<0.08&&R.gold.ph>-0.05&&R.gold.ph<0.05&&R.gold.dp>-0.02&&R.gold.dp<0&&P.upQadcR>%f)",upADCcut);
      ...
      TH1F* hQsq = new TH1F("hQsq", Form("RHRS Qsq w/ T1 trigger (run%d,%s);Qsq (GeV/c)^{2}",run_num,target.Data()), 100, 0.002, 0.013);
      TH1F* hQsq_cal = new TH1F("hQsq_cal",Form("RHRS Qsq w/ T1 trigger (run%d,%s);Qsq (GeV/c)^{2}",run_num,target.Data()), 100, 0.002, 0.013);
      ...
      c1->SaveAs(Form("./temp/RHRS_Qsq_comp_%s_run%d.pdf",target.Data(),run_num));
    }
