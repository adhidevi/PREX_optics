...
//uses ADC cut (fixed) and vary scattering angle cut
void AsymForAccCut(int run,TString date,TString target){
  ...
  //For the interpolation
   ...
   //plot procected x, y, and adc
      ...
      gPad->SetGridx();
      gPad->SetGridy();
      TH1F* huq = new TH1F("huq", Form("ADC raw (run%d, detZ = 1.3 m, %s);ADC raw;Events/CH",run,target.Data()), 300,400,700);
      huq->SetLineColor(1);
  ...
  for(long i = 0; i < n; i++){ 
    ...
    //VDC cut and x cut
     if( thisu1==1 && thisv1==1 && thisu2==1 && thisv2==1 && thisDet > upadc_cutL){
 ...
 Q2->SetTitle(Form("Q^{2} (GeV/c)^{2}"));
 Theta->SetTitle(Form("#theta_{lab} (deg)"));
 Asym->SetTitle(Form("Asymmetry (ppm)"));
 SAsym->SetTitle(Form("Stretched Asym. (ppm)"));
 sens->SetTitle(Form("Sensitivity"));
 ...
 gSystem->Exec(Form("pdfunite ./temp/plot*.pdf ./temp/Asym_plots_run%d.pdf",run));
 gSystem->Exec(Form("rm -rf ./temp/plot*.pdf"));

 ofstream outfile(Form("./TextFiles/output_adcCut_allRuns.csv"),ios_base::app);
 outfile<<run<<"\t"<<date.Data()<<"\t"<<target.Data()<<"\t"<<Theta->GetMean()<<"\t"<<Theta->GetMeanError()<<"\t"<<Asym->GetMean()<<"\t"<<Asym->GetMeanError()<<"\t"<<SAsym->GetMean()<<"\t"<<SAsym->GetMeanError()<<"\t"<<Q2->GetMean()<<"\t"<<Q2->GetMeanError()<<"\t"<<sens->GetMean()<<"\t"<<sens->GetMeanError()<<endl;
 outfile.close();
...
//      xCut_line->Draw();
  ...
  for(long i = 0; i < n; i++){ 
    ...
    //VDC cut and ADC cut
     if( thisu1==1 && thisv1==1 && thisu2==1 && thisv2==1 && thisDet > upadc_cutR){
 ...
 gSystem->Exec(Form("pdfunite ./temp/*.pdf ./plots/plots_adcCut_run%d.pdf",run));
 gSystem->Exec(Form("rm -rf ./temp/*.pdf"));

 ofstream outfile(Form("./TextFiles/output_adcCut_allRruns.csv"),ios_base::app);
 outfile<<run<<"\t"<<date.Data()<<"\t"<<target.Data()<<"\t"<<Theta->GetMean()<<"\t"<<Theta->GetMeanError()<<"\t"<<Asym->GetMean()<<"\t"<<Asym->GetMeanError()<<"\t"<<SAsym->GetMean()<<"\t"<<SAsym->GetMeanError()<<"\t"<<Q2->GetMean()<<"\t"<<Q2->GetMeanError()<<"\t"<<sens->GetMean()<<"\t"<<sens->GetMeanError()<<endl;
 outfile.close();
